 ...
 * @ingroup libcsupport
 *
 * @brief  open() - POSIX 1003.1 5.3.1 - Open a File
 *
...
)
{
  ...
  status = epos_filesystem_evaluate_path(
  ...
  if ( status == -1 ) {
    ...
    status = epos_filesystem_evaluate_path( pathname, strlen( pathname ), 0x0, &loc, true );
  ...
  iop->flags     |= epos_libio_fcntl_flags( flags );
  iop->pathinfo   = loc;
  ...
  if ( (flags & O_TRUNC) == O_TRUNC ) {
    rc = ftruncate( iop - epos_libio_iops, 0 );
    if ( rc ) {
      if(errno) rc = errno;
      close( iop - epos_libio_iops );
  ...
  if ( rc ) {
    ...
    epos_set_errno_and_return_minus_one( rc );
  }

  return iop - epos_libio_iops;
}
