 ...
 * @ingroup libcsupport
 *
 * @brief   RTEMS Malloc Block Boundary Integrity Checker
 *
...
static void epos_malloc_boundary_initialize(void)
{
  mallocNodeHead.back = &mallocNodeHead;
  mallocNodeHead.forw = &mallocNodeHead;
}

static uint32_t epos_malloc_boundary_overhead(void)
{
  return sizeof(struct mallocNode) + SENTINELSIZE;
}

static void epos_malloc_boundary_at_malloc(
  void     *pointer,
  size_t    size
)
{
...
static void epos_malloc_boundary_at_free(
  void     *pointer
)
{
...
static void epos_malloc_boundary_at_realloc(
  void     *pointer ,
  size_t    size 
)
{
 ...
 */
epos_malloc_boundary_functions_t epos_malloc_boundary_functions_table = {
  epos_malloc_boundary_initialize,
  epos_malloc_boundary_overhead,
  epos_malloc_boundary_at_malloc,
  epos_malloc_boundary_at_free,
  epos_malloc_boundary_at_realloc,
};

epos_malloc_boundary_functions_t *epos_malloc_boundary_helpers = NULL;
/*   &epos_malloc_boundary_functions_table; */
