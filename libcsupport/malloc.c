...
)
{
  ...
  if ( !return_this ) {
    if (epos_malloc_sbrk_helpers)
      return_this = (*epos_malloc_sbrk_helpers->extend)( size );
    if ( !return_this ) {
      errno = ENOMEM;
      return (void *) 0;
    }
   ...
   */
  if ( epos_malloc_dirty_helper )
    (*epos_malloc_dirty_helper)( return_this, size );

  /*
   *  If configured, update the statistics
   */
  if ( epos_malloc_statistics_helpers )
    (*epos_malloc_statistics_helpers->at_malloc)(return_this);
