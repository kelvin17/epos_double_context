 ...
 * @ingroup libcsupport
 *
 * @brief   fcntl() - POSIX 1003.1b 6.5.2 - File Control
 *
...
)
{
  ...
  switch ( cmd ) {
    case F_DUPFD:        /* dup */
      fd2 = va_arg( ap, int );
      if ( fd2 )
        diop = epos_libio_iop( fd2 );
      else {
        /* allocate a file control block */
        diop = epos_libio_allocate();
        if ( diop == 0 ) {
          ret = -1;
          break;
        }
    ...
    case F_GETFL:        /* more flags (cloexec) */
      ret = epos_libio_to_fcntl_flags( iop->flags );
      break;

    case F_SETFL:
      flags = epos_libio_fcntl_flags( va_arg( ap, int ) );
