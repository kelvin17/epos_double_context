 ...
 * @ingroup libcsupport
 *
 * @brief  unlink() - POSIX 1003.1b - 5.5.1 - Remove an existing link
 */
...
)
{
  int                               parentpathlen;
  const char                       *name;
  epos_filesystem_location_info_t  parentloc;
  ...
  parentpathlen = epos_filesystem_dirname ( path );

  if ( parentpathlen == 0 )
    epos_filesystem_get_start_loc( path, &i, &parentloc );
  else {
    result = epos_filesystem_evaluate_path( path, parentpathlen,
  ...
  if ( result != 0 ) {
    if ( free_parentloc )
      epos_filesystem_freenode( &parentloc );
    return -1;
  }
