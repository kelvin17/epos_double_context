 ...
 * Default pathconfs.
 */
const epos_filesystem_limits_and_options_t epos_filesystem_default_pathconf = {
   5,    /* link_max: count */
...
static epos_filesystem_mount_table_entry_t *alloc_mount_table_entry(
...
)
{
  ...
  size_t size = sizeof( epos_filesystem_mount_table_entry_t )
    + filesystemtype_size + source_size + target_length + 1;
  epos_filesystem_mount_table_entry_t *mt_entry = calloc( 1, size );
...
{
  epos_filesystem_fsmount_me_t mount_h = NULL;
  epos_filesystem_location_info_t      loc;
  epos_filesystem_mount_table_entry_t *mt_entry = NULL;
  epos_filesystem_location_info_t     *loc_to_free = NULL;
   ...
   *  Get mount handler
   */
  mount_h = epos_filesystem_get_mount_handler( filesystemtype );
  if ( !mount_h )
    epos_set_errno_and_return_minus_one( EINVAL );
  ...
  mt_entry->pathconf_limits_and_options = epos_filesystem_default_pathconf;
  ...
  if ( has_target ) {
    if ( epos_filesystem_evaluate_path(
    ...
    if ( epos_filesystem_mount_iterate( is_node_fs_root, loc.node_access ) ) {
      errno = EBUSY;
      goto cleanup_and_bail;
    }
     ...
     */
    if ( !epos_chain_is_empty( &mount_chain ) ) {
      errno = EINVAL;
      goto cleanup_and_bail;
    }
   ...
   */
  epos_libio_lock();
  epos_chain_append( &mount_chain, &mt_entry->Node );
  epos_libio_unlock();

  if ( !has_target )
    epos_filesystem_root = mt_entry->mt_fs_root;
...
cleanup_and_bail:
  ...
  if ( loc_to_free )
    epos_filesystem_freenode( loc_to_free );

  return -1;
}

bool epos_filesystem_mount_iterate(
...
)
{
  ...
  epos_libio_lock();
  for (
    node = epos_chain_first( &mount_chain );
    !epos_chain_is_tail( &mount_chain, node ) && !stop;
    node = epos_chain_next( node )
  ) {
    const epos_filesystem_mount_table_entry_t *mt_entry =
      (epos_filesystem_mount_table_entry_t *) node;
