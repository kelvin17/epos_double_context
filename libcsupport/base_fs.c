 ...
 *  by the structure epos_filesystem_mount_table.  The usual
...
{
  int                                   status;
  const epos_filesystem_mount_table_t *mt;
  ...
  epos_filesystem_umask = 022;

  /*
   *  mount the first filesystem.
   */
 
  if ( epos_filesystem_mount_table_size == 0 )
    epos_fatal_error_occurred( 0xABCD0001 );

  mt = &epos_filesystem_mount_table[0];

  status = mount( mt->device, mt->mount_point, mt->type, mt->fsoptions, NULL );

  if ( status == -1 )
    epos_fatal_error_occurred( 0xABCD0002 );
  ...
  if ( status != 0 )
    epos_fatal_error_occurred( 0xABCD0003 );
