 ...
 * @ingroup libcsupport
 *
 * @brief unmount() - Unmount a File System
 */
...
)
{
  ...
  epos_filesystem_location_info_t     *fs_root_loc;
  epos_filesystem_location_info_t     *fs_mount_loc;
  ...
  if ( epos_filesystem_current.mt_entry == mt_entry )
    epos_set_errno_and_return_minus_one( EBUSY );

  /*
   *  Verify there are no file systems below the path specified
   */

  if ( epos_filesystem_mount_iterate( is_fs_below_mount_point,
  ...
  if ( epos_libio_is_open_files_in_fs( mt_entry ) == 1 )
    epos_set_errno_and_return_minus_one( EBUSY );
  ...
  epos_libio_lock();
  epos_chain_extract( &mt_entry->Node );
  epos_libio_unlock();
  ...
  epos_filesystem_freenode( fs_mount_loc );
  free( mt_entry );
