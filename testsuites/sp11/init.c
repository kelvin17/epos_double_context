...
)
{
  ...
  Timer_name[ 4 ] = epos_build_name( 'T', 'M', '4', ' ' );
  Timer_name[ 5 ] = epos_build_name( 'T', 'M', '5', ' ' );
  Timer_name[ 6 ] = epos_build_name( 'T', 'M', '6', ' ' );
  ...
  directive_failed( status, "epos_timer_create of TM2" );

  status = epos_timer_create( Timer_name[ 3 ], &Timer_id[ 3 ] );
  directive_failed( status, "epos_timer_create of TM3" );

  status = epos_timer_create( Timer_name[ 4 ], &Timer_id[ 4 ] );
  directive_failed( status, "epos_timer_create of TM4" );

  status = epos_timer_create( Timer_name[ 5 ], &Timer_id[ 5 ] );
  directive_failed( status, "epos_timer_create of TM5" );

  status = epos_timer_create( Timer_name[ 6 ], &Timer_id[ 6 ] );
  directive_failed( status, "epos_timer_create of TM6" );
