...
)
{
  ...
  Task_name[ 6 ] =  epos_build_name( 'F', 'P', '1', ' ' );
  ...
  directive_failed( status, "epos_task_create of FP1" );

  status = epos_task_start( Task_id[ 6 ], First_FP_task, 0 );
  directive_failed( status, "epos_task_start of FP1" );

  status = epos_task_start( Task_id[ 1 ], Task_1, 0 );
  directive_failed( status, "epos_task_start of TA1" );

  status = epos_task_start( Task_id[ 2 ], Task_1, 0 );
  directive_failed( status, "epos_task_start of TA2" );

  status = epos_task_start( Task_id[ 3 ], Task_1, 0 );
  directive_failed( status, "epos_task_start of TA3" );

  status = epos_task_start( Task_id[ 4 ], FP_task, 0 );
  directive_failed( status, "epos_task_start of TA4" );

  status = epos_task_start( Task_id[ 5 ], FP_task, 0 );
  directive_failed( status, "epos_task_create of TA5" );
