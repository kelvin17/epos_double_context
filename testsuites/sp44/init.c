...
epos_id TaskA_id, TaskB_id;

#define TEST_FAILED(_status, _msg) \
  do { \
    printk("Test failed, status code: %d, msg: %s\n", _status, _msg); \
    exit(1); \
  } while(0)

/* TASK A/B */
epos_task TaskAB_entry(epos_task_argument me)
{
  static epos_mode previous_mode_set;
  epos_status_code status;
  uint32_t iterations = 0;

  status = epos_task_mode(RTEMS_PREEMPT | RTEMS_TIMESLICE,
  ...
  while(1) {
    if (turn == me) {
      printk("Task #%d's turn. Now setting turn to %d\n", me, 1 - me);
      turn = 1 - me;
  
      if ( ++iterations == 10 ) {
        printk( "*** END OF SP44 TEST ***" );
        exit( 0 );
      }
    }
  }
...
{
  static epos_status_code status;

  printk( "\n\n*** SP44 TEST ***" );

  /* Create Task A */
  status = epos_task_create(TASK_A_NAME, TASK_A_PRIORITY, TASK_A_STACKSIZE,
         TASK_A_INITMODE, TASK_A_MODEATTR, &TaskA_id);

  if (status != RTEMS_SUCCESSFUL)
    TEST_FAILED(status,"epos_task_create failed.\n");

  /* Start Task A */
  status = epos_task_start(TaskA_id, TaskAB_entry, 0);
  if (status != RTEMS_SUCCESSFUL)
    TEST_FAILED(status,"epos_task_start failed.\n");

  /* Create Task B */
  status = epos_task_create(TASK_B_NAME, TASK_B_PRIORITY, TASK_B_STACKSIZE,
  ...
  /* Start Task B */
  status = epos_task_start(TaskB_id, TaskAB_entry, 1);
  if (status != RTEMS_SUCCESSFUL)
    TEST_FAILED(status,"epos_task_start failed.\n");
  ...
  if (status != RTEMS_SUCCESSFUL)
    TEST_FAILED(status,"epos_task_suspend failed.\n");
