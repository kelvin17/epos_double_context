 ...
 *
 *  @brief RTEMS对象类信息（对象ID，名字）管理服务模块。
 *
...
} epos_object_api_class_information;
 ...
 */
#define epos_build_id( _api, _class, _node, _index ) \
 ...
 *
 */
#define epos_build_name( _C1, _C2, _C3, _C4 ) \
 ...
 */
epos_status_code epos_object_get_classic_name(
  epos_id      id,
  epos_name   *name
);
 ...
 *  @return error code - if unsuccessful
 */
epos_status_code epos_object_set_name(
  epos_id       id,
  const char    *name
);
 ...
 *  @note A body is also provided.
 */
#define epos_object_id_get_api( _id ) \
 ...
 *  @note A body is also provided.
 */
#define epos_object_id_get_class( _id ) \
 ...
 *  @note A body is also provided.
 */
#define epos_object_id_get_node( _id ) \
 ...
 *  @note A body is also provided.
 */
#define epos_object_id_get_index( _id ) \
 ...
 *  @note A body is also provided.
 */
#define epos_object_id_api_minimum() \
 ...
 *  @note A body is also provided.
 */
#define epos_object_id_api_maximum() \
  OBJECTS_APIS_LAST
 ...
 */
int epos_object_api_minimum_class(
  int api
);
 ...
 *          class number for the specified @a api.
 */
int epos_object_id_api_maximum_class(
  int api
);
 ...
 *
 */
epos_status_code epos_object_get_class_information(
  int                                 the_api,
  int                                 the_class,
