 ...
 * @file
 *
 * @ingroup epos_interrupt_extension
 *
...
/**
 * @defgroup epos_interrupt_extension Interrupt Manager Extension
 *
 * @ingroup ClassicINTR
 *
 *  @brief RTEMS中断管理扩展服务模块。
 *
 ...
 */
#define RTEMS_INTERRUPT_UNIQUE ((epos_option) 0x00000001)

/**
 * @brief Allows that this interrupt handler may share a common interrupt
 * vector with other handler.
 */
#define RTEMS_INTERRUPT_SHARED ((epos_option) 0x00000000)
 ...
 */
typedef void (*epos_interrupt_handler)(void *);
 ...
 */
epos_status_code epos_interrupt_handler_install(
  epos_vector_number vector,
  const char *info,
  epos_option options,
  epos_interrupt_handler handler,
  void *arg
);
 ...
 * @retval * Other error states are BSP specific.
 */
epos_status_code epos_interrupt_handler_remove(
  epos_vector_number vector,
  epos_interrupt_handler handler,
  void *arg
);
 ...
 *
 * @see epos_interrupt_handler_iterate()
 */
typedef void (*epos_interrupt_per_handler_routine)(
  void *, const char *, epos_option, epos_interrupt_handler, void *
);
 ...
 * @retval * Other error states are BSP specific.
 */
epos_status_code epos_interrupt_handler_iterate(
  epos_vector_number vector,
  epos_interrupt_per_handler_routine routine,
  void *arg
);
 ...
 * installed on the server with epos_interrupt_server_handler_install() and
 * removed with epos_interrupt_server_handler_remove() using this identifier.
 ...
 * @retval * Other error states are BSP specific.
 */
epos_status_code epos_interrupt_server_initialize(
  epos_task_priority priority,
  size_t stack_size,
  epos_mode modes,
  epos_attribute attributes,
  epos_id *server
);
 ...
 * This function may block.
 *
 * @see epos_interrupt_handler_install().
 *
 * @retval RTEMS_SUCCESSFUL Shall be returned in case of success.
 * @retval RTEMS_INCORRECT_STATE If the interrupt handler server is not
 * initialized this shall be returned.
 * @retval * For other errors see epos_interrupt_handler_install().
 */
epos_status_code epos_interrupt_server_handler_install(
  epos_id server,
  epos_vector_number vector,
  const char *info,
  epos_option options,
 ...
 * This function may block.
 *
 * @see epos_interrupt_handler_remove().
 *
 * @retval RTEMS_SUCCESSFUL Shall be returned in case of success.
 * @retval RTEMS_INCORRECT_STATE If the interrupt handler server is not
 * initialized this shall be returned.
 * @retval * For other errors see epos_interrupt_handler_remove().
 */
epos_status_code epos_interrupt_server_handler_remove(
  epos_id server,
  epos_vector_number vector,
  epos_interrupt_handler handler,
  void *arg
