 ...
 *  @ingroup EposIMFS
 *
 *  @brief IMFS Directory Access Routines.
 */
...
ssize_t imfs_dir_read(
  epos_libio_t  *iop,
  char           *buffer,//void to char ,by zwj
  size_t          count
)
{
  /*
   ...
   */
   ...
   if ( epos_chain_is_empty( the_chain ) )
      return 0;
   ...
   for (
      ...
      if ( epos_chain_is_tail( the_chain, the_node ) ){
...
)
{
   ...
   for ( the_node = the_chain->first ;
         !epos_chain_is_tail( the_chain, the_node ) ;
         the_node = the_node->next ) {
...
)
{
  ...
  if ( ! epos_chain_is_empty( &the_jnode->info.directory.Entries ) )
     epos_set_errno_and_return_minus_one( ENOTEMPTY );
  ...
  if ( !epos_libio_is_file_open( the_jnode ) && (the_jnode->st_nlink < 1) ) {
    ...
    /*
     * Is the epos_filesystem_current is this node?
     */
