 ...
 *  @ingroup EposIMFS
 *
 *  @brief Evaluation IMFS Node Support Routines.
 */
...
int IMFS_evaluate_permission(
  epos_filesystem_location_info_t  *node,
  int                                flags
)
{
  uid_t         st_uid;
  ...
  if ( !epos_libio_is_valid_perms( flags ) )
    epos_set_errno_and_return_minus_one( EPERM );
...
)
{
  ...
  if ( jnode->type != IMFS_HARD_LINK )
    epos_fatal_error_occurred (0xABCD0000);
  ...
  if ( !IMFS_evaluate_permission( node, flags ) )
    epos_set_errno_and_return_minus_one( EACCES );
...
)
{
  ...
  if ( !jnode->Parent )
    epos_fatal_error_occurred( 0xBAD00000 );
...
)
{
  ...
  do {
    ...
    epos_filesystem_link_counts ++;
    if ( epos_filesystem_link_counts > MAXSYMLINK ) {
      epos_filesystem_link_counts = 0;
      epos_set_errno_and_return_minus_one( ELOOP );
    }
  ...
  while( !done ) {
    ...
    switch( type ) {
      ...
      case IMFS_UP_DIR:
       ...
       if ( pathloc->node_access == epos_filesystem_root.node_access )
      ...
      case IMFS_NO_MORE_PATH:
        epos_set_errno_and_return_minus_one( EEXIST );
        break;

      case IMFS_INVALID_TOKEN:
        epos_set_errno_and_return_minus_one( ENAMETOOLONG );
        break;
                   ...
                   )
{
  ...
  if ( !epos_libio_is_valid_perms( flags ) ) {
    assert( 0 );
    epos_set_errno_and_return_minus_one( EIO );
  }
