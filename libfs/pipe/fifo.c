...
extern uint16_t epos_pipe_no;
static epos_id epos_pipe_semaphore = 0;
extern bool epos_pipe_configured;
...
#define PIPE_LOCK(_pipe)  \
  ( epos_semaphore_obtain(_pipe->Semaphore, RTEMS_WAIT, RTEMS_NO_TIMEOUT)  \
   == RTEMS_SUCCESSFUL )

#define PIPE_UNLOCK(_pipe)  epos_semaphore_release(_pipe->Semaphore)

#define PIPE_READWAIT(_pipe)  \
  ( epos_barrier_wait(_pipe->readBarrier, RTEMS_NO_TIMEOUT)  \
   == RTEMS_SUCCESSFUL)

#define PIPE_WRITEWAIT(_pipe)  \
  ( epos_barrier_wait(_pipe->writeBarrier, RTEMS_NO_TIMEOUT)  \
   == RTEMS_SUCCESSFUL)

#define PIPE_WAKEUPREADERS(_pipe) \
  do {uint32_t n; epos_barrier_release(_pipe->readBarrier, &n); } while(0)

#define PIPE_WAKEUPWRITERS(_pipe) \
  do {uint32_t n; epos_barrier_release(_pipe->writeBarrier, &n); } while(0)
 ...
 * Called with epos_pipe_semaphore held.
 */
...
)
{
  ...
  err = -ENOMEM;
  if (epos_barrier_create(
        epos_build_name ('P', 'I', 'r', c),
        RTEMS_BARRIER_MANUAL_RELEASE, 0,
        &pipe->readBarrier) != RTEMS_SUCCESSFUL)
    goto err_rbar;
  if (epos_barrier_create(
        epos_build_name ('P', 'I', 'w', c),
        RTEMS_BARRIER_MANUAL_RELEASE, 0,
        &pipe->writeBarrier) != RTEMS_SUCCESSFUL)
    goto err_wbar;
  if (epos_semaphore_create(
        epos_build_name ('P', 'I', 's', c), 1,
...
err_sem:
  epos_barrier_delete(pipe->writeBarrier);
err_wbar:
  epos_barrier_delete(pipe->readBarrier);
err_rbar:
  free(pipe->Buffer);
err_buf:
  free(pipe);
...
)
{
  epos_barrier_delete(pipe->readBarrier);
  epos_barrier_delete(pipe->writeBarrier);
  epos_semaphore_delete(pipe->Semaphore);
  free(pipe->Buffer);
  free(pipe);
}
...
)
{
  ...
  if (epos_semaphore_obtain(epos_pipe_semaphore,
...
out:
  epos_semaphore_release(epos_pipe_semaphore);
  return err;
}
...
)
{
  ...
  epos_status_code sc;
  sc = epos_semaphore_obtain(pipe->Semaphore,
  ...
  sc = epos_semaphore_obtain(epos_pipe_semaphore,
                              RTEMS_WAIT, RTEMS_NO_TIMEOUT);
 ...
 */
void epos_pipe_initialize (void)
{
  if (!epos_pipe_configured)
    return;

  if (epos_pipe_semaphore)
    return;

  epos_status_code sc;
  sc = epos_semaphore_create(
        epos_build_name ('P', 'I', 'P', 'E'), 1,
        RTEMS_BINARY_SEMAPHORE | RTEMS_INHERIT_PRIORITY | RTEMS_PRIORITY,
        RTEMS_NO_PRIORITY, &epos_pipe_semaphore);
  if (sc != RTEMS_SUCCESSFUL)
    epos_fatal_error_occurred (sc);
