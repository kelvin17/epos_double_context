...
Context_Control *GetExecutingRegs()
{
	return _Thread_Executing->Registers;
}

Context_Control *GetHeirRegs()
{
	return _Thread_Heir->Registers;
}


void Set_Executing(void)
{
...
{ 
    uint32_t *sp = stack_base+size-1;
	uint32_t *bp = stack_base+size-1;
 ...
 *  XXX document implementation including references if appropriate
 */
uint32_t bwdsp_get_gcsr(void);

uint32_t   _CPU_ISR_Get_level( void )
{
  uint32_t   level;
  level = bwdsp_get_gcsr();
  level = (level & 0x01) ? 0 : 1;
  return level;
}
