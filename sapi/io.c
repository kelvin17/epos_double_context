...
epos_driver_address_table *_IO_Driver_address_table;

void _IO_Manager_initialization(void)
{
  uint32_t                    index;
  epos_driver_address_table *driver_table;
  ...
  _IO_Driver_address_table = (epos_driver_address_table *)
      _Workspace_Allocate_or_fatal_error(
        sizeof( epos_driver_address_table ) * ( number_of_drivers )
      );
  _IO_Number_of_drivers = number_of_drivers;
...
void _IO_Initialize_all_drivers( void )
{
   ...
   for ( major=0 ; major < _IO_Number_of_drivers ; major ++ )
     (void) epos_io_initialize( major, 0, NULL );
}
