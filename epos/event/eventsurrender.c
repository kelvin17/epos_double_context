...
)
{
  ...
  epos_event_set     seized_events;
  epos_option        option_set;
  RTEMS_API_Control  *api;

  api = the_thread->API_Extensions[ THREAD_API_RTEMS ];

  option_set = (epos_option) the_thread->Wait.option;

  _ISR_Disable( level );
  pending_events  = api->pending_events;
  event_condition = (epos_event_set) the_thread->Wait.count;
  ...
  if ( _ISR_Is_in_progress() &&
    ...
    if ( seized_events == event_condition || _Options_Is_any(option_set) ) {
      ...
      *(epos_event_set *)the_thread->Wait.return_argument = seized_events;
