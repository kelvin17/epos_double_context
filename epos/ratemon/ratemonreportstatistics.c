...
)
{
  ...
  epos_id                               id;
  epos_rate_monotonic_period_statistics the_stats;
  epos_rate_monotonic_period_status     the_status;
  char                                   name[5];
...
*/
  ...
  for ( id=_Rate_monotonic_Information.minimum_id ;
        ...
        id++ ) {
    status = epos_rate_monotonic_get_statistics( id, &the_stats );
    if ( status != RTEMS_SUCCESSFUL )
      continue;

    /* If the above passed, so should this but check it anyway */
    status = epos_rate_monotonic_get_status( id, &the_status );
    #if defined(RTEMS_DEBUG)
      if ( status != RTEMS_SUCCESSFUL )
        continue;
    #endif

    epos_object_get_name( the_status.owner, sizeof(name), name );
...
{
  epos_rate_monotonic_report_statistics_with_plugin( NULL, printk_plugin );
}
