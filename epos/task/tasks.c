...
)
{
  ...
  if ( !epos_configuration_get_notepads_enabled() )
...
)
{
  ...
  while (tvp) {
    next = (epos_task_variable_t *)tvp->next;
...
)
{
  ...
  asr->nest_level += 1;
  epos_task_mode( asr->mode_set, RTEMS_ALL_MODE_MASKS, &prev_mode );

  (*asr->handler)( signal_set );

  asr->nest_level -= 1;
  epos_task_mode( prev_mode, RTEMS_ALL_MODE_MASKS, &prev_mode );
