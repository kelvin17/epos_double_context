 ...
 *  @file timerserver.c
 *
 *  Timer Manager - epos_timer_initiate_server directive along with
 ...
 */
static epos_task _Timer_server_Body(
  epos_task_argument arg
)
{
...
)
{
  ...
  status = epos_task_start(
    id,
    _Timer_server_Body,
    (epos_task_argument) ts
  );

  #if defined(RTEMS_DEBUG)
    /*
     *  One would expect a call to epos_task_delete() here to clean up
