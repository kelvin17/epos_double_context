...
)
{
  ...
  put( "TA1 - rtems_partition_ident - partition 1 id = " );
 ...
 // printf( "%08" PRIxepos_id "\n", ptid_1 );
  ...
  put( "TA1 - rtems_partition_ident - partition 2 id = " );
  status = epos_partition_ident(
    Partition_name[ 2 ],
    RTEMS_SEARCH_ALL_NODES,
    &ptid_2
  ...
  put("TA1 - rtems_partition_get_buffer - buffer 1 from partition 1  - "  );
 ...
 // new_line;
  ...
  put("TA1 - rtems_partition_get_buffer - buffer 2 from partition 1  - "  );
...
//  new_line;
  ...
  put("TA1 - rtems_partition_get_buffer - buffer 3 from partition 2  - "  );
...
//  new_line;
  ...
  put("TA1 - rtems_partition_get_buffer - buffer 4 from partition 2  - "  );
...
//  new_line;
  ...
  put("TA1 - rtems_partition_get_buffer - attempt to get buffer 5 from partition 2"  );
  status = epos_partition_get_buffer( ptid_2, &buffer_address_5 );
  //directive_failed( status, "rtems_partition_get_buffer" );
  if(status == RTEMS_UNSATISFIED)
  	printk("Error code :RTEMS_UNSATISFIED.For all buffers are allocated!\n");

  put("TA1 - rtems_partition_return_buffer - buffer 1 to partition 1 - "  );
  Put_address_from_area_1( buffer_address_1 );
//  new_line;
  ...
  put("TA1 - rtems_partition_return_buffer - buffer 2 to partition 1 - "  );
  Put_address_from_area_1( buffer_address_2 );
//  new_line;
...
//  new_line;
 ...
 // new_line;
  ...
  put( "TA1 - rtems_partition_delete - delete partition 1"  );
  status = epos_partition_delete( ptid_1 );
  //directive_failed( status, "rtems_partition_delete" );
  directive_suc(status);

  put( "TA1 - rtems_partition_delete - delete partition 2"  );
  status = epos_partition_delete( ptid_2 );
  //directive_failed( status, "rtems_partition_delete" );
  directive_suc(status);
  put( "*** END OF TEST Partition ***" );
 // epos_test_exit(0);
}
