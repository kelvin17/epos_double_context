...
)
{
  ...
  first_block->size_and_flag = first_block_size | HEAP_PREV_BLOCK_USED;//////////////error
 ...
 // last_block = _Heap_Block_at( first_block, first_block_size ); 函数返回值传递寄存器问题 by qyq
	 last_block = (Heap_Block *) ((uintptr_t) first_block + first_block_size);
