 ...
 *  @ingroup ScoreThread
 *
 *  @brief The include file of the thread control block.
 *
...
# error "Never use "thread.inl" directly; include "thread.h" instead."
#endif
...
RTEMS_INLINE_ROUTINE void _Thread_Disable_dispatch( void )
{
   ...
   */
    ...
    if (_System_state_Is_up(_System_state_Get()) && (_ISR_Nest_level == 0)) {
      if ( epos_stack_checker_is_blown() ) {
	printk( "Stack blown!!\n" );
	epos_fatal_error_occurred( 99 );
      }
    }
  #endif
...
RTEMS_INLINE_ROUTINE void _Thread_Enable_dispatch( void )
{
  ...
  if ( _Thread_Dispatch_disable_level == 0)
	_Thread_Dispatch();
  else if ( (--_Thread_Dispatch_disable_level) == 0 )
    _Thread_Dispatch();
}
#endif
